...
func _gen_x_file(table_path, table_file, dest_path, file_suffix string, tt *this_table) error {
	...
	if file_suffix == ".go" {
		...
		return errors.New("not supported suffix")
	}
...
func GenSourceAndCsv(excel_path, excel_file, src_dest_path, csv_dest_path string, header_index, value_type_index, user_type_index, data_start_index int32) error {
	...
	for idx := 0; idx < sheet.MaxCol; idx++ {
		...
		if header_index < int32(sheet.MaxRow) {
			...
			if c == nil {
				gslog.Warn("cell row[%v] col[%v] is null", header_index, idx)
				continue
			}
		...
		if value_type_index < int32(sheet.MaxRow) {
			...
			if c == nil {
				gslog.Warn("cell row[%v] col[%v] is null", value_type_index, idx)
				continue
			}
			...
			if c.Value == "float" || c.Value == "float32" {
				...
				gslog.Warn("value type %v invalid in column %v", c.Value, idx)
				continue
			}

			// 第一列类型必须是int
			if idx == 0 && (col.value_type != "int32" && col.value_type != "int64") {
				return errors.New("first column data type must be int category")
			}
		}
		if user_type_index < int32(sheet.MaxRow) {
			c := sheet.Cell(int(user_type_index), idx)
			if c == nil {
				gslog.Warn("cell row[%v] col[%v] is null", user_type_index, idx)
				continue
			}
			if !(c.Value == "c|s" || c.Value == "s") {
				if c.Value != "c" {
					gslog.Warn("unsupported user type %v", c.Value)
				}
				continue
			}
	...
	if tt.cols == nil || len(tt.cols) == 0 {
		return fmt.Errorf("table %v no columns", excel_file)
	}
	...
	// gen source
	if err = _gen_x_file(excel_path, excel_file, src_dest_path, ".go", &tt); err != nil {
		return err
	}

	// gen csv
	if err = _gen_x_file(excel_path, excel_file, csv_dest_path, ".csv", &tt); err != nil {
		return err
	}
